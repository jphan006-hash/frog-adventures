const canvas = document.getElementById("game");
const ctx = canvas.getContext("2d");
const ui = document.getElementById("ui");

let players = [];
let bodies = [];
let meeting = false;

// COLORS FOR 3 FROG KILLERS 🐸
const frogColors = ["lime", "yellow", "magenta"];

// CREATE PLAYERS
for (let i = 0; i < 16; i++) {
    players.push({
        id: i,
        x: Math.random()*580,
        y: Math.random()*580,
        alive: true,
        role: "detective",
        color: "white"
    });
}

// ASSIGN 3 KILLERS WITH DIFFERENT COLORS
let shuffled = [...players].sort(() => Math.random()-0.5);
for (let i = 0; i < 3; i++) {
    shuffled[i].role = "killer";
    shuffled[i].color = frogColors[i]; // each frog different color
}

// YOU ARE PLAYER 0
let you = players[0];

// MOVEMENT
document.addEventListener("keydown", e => {
    if (!you.alive || meeting) return;

    if (e.key === "ArrowUp") you.y -= 5;
    if (e.key === "ArrowDown") you.y += 5;
    if (e.key === "ArrowLeft") you.x -= 5;
    if (e.key === "ArrowRight") you.x += 5;
});

// KILL (SPACE)
document.addEventListener("keydown", e => {
    if (e.key === " " && you.role === "killer" && !meeting && you.alive) {
        players.forEach(p => {
            if (p !== you && p.alive && dist(you,p) < 20) {
                p.alive = false;
                bodies.push({...p});
            }
        });
    }
});

// DISTANCE
function dist(a,b){
    return Math.hypot(a.x-b.x, a.y-b.y);
}

// AI
function updateAI(){
    players.forEach(p=>{
        if (!p.alive || p===you) return;

        // random movement
        p.x += (Math.random()-0.5)*2;
        p.y += (Math.random()-0.5)*2;

        // killer AI
        if (p.role==="killer" && Math.random()<0.01){
            let targets = players.filter(t=>t.alive && t!==p);
            let target = targets[Math.floor(Math.random()*targets.length)];
            if (target){
                target.alive=false;
                bodies.push({...target});
            }
        }
    });
}

// AUTO REPORT
function checkBodies(){
    for (let b of bodies){
        if (dist(you,b) < 25 && !meeting){
            startMeeting();
        }
    }
}

// MEETING
function startMeeting(){
    meeting = true;
    ui.innerHTML = "<h3>Meeting! Vote someone:</h3>";

    players.forEach(p=>{
        if (p.alive){
            let btn = document.createElement("button");
            btn.innerText = "Player " + p.id;
            btn.onclick = ()=>vote(p);
            ui.appendChild(btn);
        }
    });
}

// VOTE
function vote(p){
    p.alive = false;
    meeting = false;
    ui.innerHTML = "";
    checkWin(p);
}

// WIN CHECK
function checkWin(voted){
    if (voted.role !== "killer"){
        alert("❌ Wrong vote! Killers win!");
        location.reload();
        return;
    }

    let killersAlive = players.filter(p=>p.role==="killer" && p.alive).length;
    let othersAlive = players.filter(p=>p.role!=="killer" && p.alive).length;

    if (killersAlive === 0){
        alert("🎉 Detectives win!");
        location.reload();
    }

    if (killersAlive >= othersAlive){
        alert("💀 Killers win!");
        location.reload();
    }
}

// DRAW
function draw(){
    ctx.clearRect(0,0,600,600);

    players.forEach(p=>{
        if (!p.alive) return;

        // YOU = blue
        if (p === you) {
            ctx.fillStyle = "cyan";
        } else {
            ctx.fillStyle = p.color;
        }

        ctx.fillRect(p.x, p.y, 10, 10);
    });

    // bodies
    bodies.forEach(b=>{
        ctx.fillStyle="red";
        ctx.fillRect(b.x, b.y, 10, 10);
    });
}

// LOOP
function loop(){
    updateAI();
    checkBodies();
    draw();
    requestAnimationFrame(loop);
}

loop();
